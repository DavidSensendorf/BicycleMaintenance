...
public class JdbcBicycleDao implements BicycleDao {
    ...
    public boolean updateBicycle(Bicycle updatedBicycle) {
        String sql = "UPDATE bicycle " +
                ...
                "WHERE bicycle_id = ? AND active = true;";
        try {
            int updated = jdbcTemplate.update(sql, updatedBicycle.getBicycleName(), updatedBicycle.getDescription(), updatedBicycle.getBicycleId());
            if (updated == 0){
                return false;
            }
        } catch (Exception e){
    ...
    public boolean deleteBicycle(int bicycleId) {
        String sql = "UPDATE bicycle " +
                ...
                "WHERE bicycle_id = ?;";
        try{
            int deleted = jdbcTemplate.update(sql, LocalDateTime.now(), bicycleId);
            if (deleted == 0){
                return false;
            }
        }catch (Exception e) {
